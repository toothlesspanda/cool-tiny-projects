...
@NamedQueries({ 
	...
	@NamedQuery(name="Prestacao.findThisMonth", query="SELECT p FROM Prestacao p WHERE p.dataPagamento >= :inicioMes AND p.dataPagamento <= :fimMes"),
	...
	public boolean pagoEsteMes(){
		if(this.dataPagamento==null)
			return false;
		else if(this.dataPagamento.get(Calendar.YEAR) == Calendar.getInstance().get(Calendar.YEAR) && this.dataPagamento.get(Calendar.MONTH) == Calendar.getInstance().get(Calendar.MONTH))
			return true;
		else
			return false;
	}
